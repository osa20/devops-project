            ...
            steps {
                script {
                    if (isUnix()) {
                        ...
                        bat 'docker run --name mysql -v ./mysql:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=mysql -e MYSQL_DATABASE=mydb -e MYSQL_USER=user -e MYSQL_PASSWORD=password -p 3309:3306 -d mysql:8.0.33'
                    }
                }
            }
        }
